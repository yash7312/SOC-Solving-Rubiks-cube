...
def key_press(k, mod):
    ...
    if k==window.key.ESCAPE: bool_do_not_quit=False  # To Quit
    if k==window.key.Q: bool_do_not_quit=False  # To Quit
    if k==window.key.LEFT:  a[0] = -1.0
    if k==window.key.RIGHT: a[0] = +1.0
    if k==window.key.UP:    a[1] = +1.0
    if k==window.key.DOWN:  a[2] = +0.8   # set 1.0 for wheels to block to zero rotation

def key_release(k, mod):
    global a
    if k==window.key.LEFT  and a[0]==-1.0: a[0] = 0
    if k==window.key.RIGHT and a[0]==+1.0: a[0] = 0
    if k==window.key.UP:    a[1] = 0
    if k==window.key.DOWN:  a[2] = 0

def run_carRacing_asHuman(policy=None, record_video=False):
    global bool_do_not_quit, a, restart
    # env = CarRacing()
    env = gym.make('CarRacing-v2').env
